...
The A365 exporter flushes on a background thread, so the token has to be deposited here
by the refresh loop and read back synchronously by the exporter's token resolver.
...
# Single process-wide store shared by the refresh loop and the exporter.
